# python도 객체지향 프로그래밍
# 클래스명은 단어의 첫 문자를 대문자로 하는 CapWords 방식으로 명명
class Rectangle:
    count = 0 # 클래스 변수
    ...
    def isSquare(w, h):
        print('정적 메서드')
    # 클래스 메소드는 객체 인스턴스를 의미하는 self 대신 cls를 가짐
    # cls 를 통해 클래스 변수에 접근 할 수 있음.
    @classmethod
    def printCount(cls):
        print('클래스 메서드')
        cls.count += 1
...
print(Rectangle.isSquare(3, 10)) # 바로 사용가능(인스턴스변수, 클래스변수에는 접근)
